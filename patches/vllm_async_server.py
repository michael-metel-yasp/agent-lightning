# cp ~/agent-lightning/patches/vllm_async_server.py \
#    ~/agent-lightning/.venv/lib/python3.12/site-packages/verl/workers/rollout/vllm_rollout/
...
class vLLMHttpServerBase:
    ...
    async def sleep(self):
        if self.rollout_mode == RolloutMode.HYBRID:
            if self.node_rank == 0:
                stale = list(self.engine.output_processor.request_states.keys())
                if stale:
                    logger.warning(f"sleep(): aborting {len(stale)} in-flight requests: {stale[:8]}")
                    await self.engine.abort(stale)
